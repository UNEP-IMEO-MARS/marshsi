# Contributing to marshsi

Thanks for contributing! `marshsi` is a **public** package, so please keep all
internal / non-public information out of commits, code, notebooks, and PRs.

## Set up the leak-prevention hooks (one time)

After `poetry install`, install the pre-commit hooks so they run automatically on
every commit:
...
They also run as part of `make check`, and on demand with:
...
Note: `docs/*_example.ipynb` are rendered into the public docs, so their outputs
are intentional — but must not contain internal paths or identifiers either.
