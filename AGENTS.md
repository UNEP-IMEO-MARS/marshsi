...
`marshsi` is published to PyPI and public on GitHub. Do not introduce — and flag
...
- **Private repositories**: do not name or link the organization's non-public
  repos. Only this package and its **public** dependencies (e.g. `georeader`)
  may be named.
...
pre-commit hooks (`gitleaks` + `mars-internal-refs`, also run by `make check`) —
