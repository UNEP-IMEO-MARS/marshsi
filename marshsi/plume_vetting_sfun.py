# https://github.com/emit-sds/plume-vetting/blob/main/sfun.py
# File copied from the above repo and modified by Vit Ruzicka and Gonzalo Mateo-Garcia
...
def get_radiance_ratio(wl, radius, num_pts, rdn, mf, orig_plume_coord, orig_points_inside_plume, ind, ite, ii, combined_mask, background_mask, plume_mask, dist_opt, rng=None, logger=None):
    ...
    if allow_repetition==0:
        row_ind, col_ind = optimize.linear_sum_assignment(similarity_matrix) # find the optimal pairs that minimize global cost (highest similarity)
    else:
