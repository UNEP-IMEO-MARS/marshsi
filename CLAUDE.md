...
`marshsi` is a matched-filter CH₄ retrieval package for EMIT / PRISMA / EnMAP.
Environment, tests and linting are documented in `README.md` and the `Makefile`.

## 🔒 This is a PUBLIC repository — never expose internal information

`marshsi` is published to PyPI and its source is public on GitHub. No commit,
...
`mars-internal-refs` scan) enforce these rules and also run as part of
`make check`. Do not bypass them with `git commit --no-verify`. Run them with
`poetry run pre-commit run --all-files`. Maintainers can list real internal
literals to block in a git-ignored `scripts/.internal-blocklist` (see the tracked
`.template`).

Note: `docs/*_example.ipynb` are rendered into the public documentation, so their
outputs are intentional — but they must never contain internal paths or
identifiers either.
